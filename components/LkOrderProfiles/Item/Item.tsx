...
import { LkOrderProfileForm } from '../../LkOrderProfiles/Form/Form';
...
export type LkOrderProfilesItemProps = {
  isExpanded?: boolean;
  isMain?: boolean;
  cities: {
    value: number;
...
export function LkOrderProfilesItem({
  isExpanded,
  isMain,
  cities,
  profile,
  ...
  return !!profile ? (
    <Accordion
      expanded={isExpanded}
      onChange={onExpand}
      sx={{ ...sx.profile, ...(isExpanded && sx.expanded )}}
    >
      <AccordionSummary expandIcon={<ExpandIcon htmlColor={theme.palette.text.muted} />} sx={sx.summary}>
        <Box sx={sx.header}>
          <Box sx={{ ...sx.locationIcon, ...(isMain && sx.mainAddress) }}><LocationIcon /></Box>

          <Typography variant="h5" sx={sx.title}>{profile?.title}</Typography>

          <Typography variant="body1" color="text.muted">{address}</Typography>
        </Box>
      </AccordionSummary>
