...
const Home: NextPage = () => {
  ...
  const selectCategory = (value: string) => setFilters({
    ...filters,
    category: filters.category !== value ? value : 'all',
    characteristics: {},
  });
  const selectCharacteristics = (key: string, selected: string[]) => setFilters({ 
    ...filters,
    characteristics: {
      [key]: selected,
    },
  ...
  const checkCharacteristics = (characteristics: { [key: string]: string }) => (
    Object.keys(filters.characteristics)
  ...
  const catalogList = (
    products?.filter(product => checkCategory(product.category.key))
